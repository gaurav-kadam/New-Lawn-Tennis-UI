...
export default function PlayerNo({ side }: PlayerNoProps) {
  ...
  const { selectedPlayer, selectPlayer, clearSelection } = useMatch();
  ...
  const handlePlayerPress = (playerNum: string) => {
    ...
    if (now - lastTap < DOUBLE_TAP_DELAY) {
      // DOUBLE TAP REGISTERED: Trigger exclusions
    ...
    <View style={{
      ...
      <View style={{
        ...
        {playerNumbers.map((item) => {
          ...
          // Dynamic layout injection styled inline with updated tokens references
