...
} from '@/components/tennis-match/hooks/useTennisMatchParams';
...
// Preserve native defaults for unstyled state text; explicit styles take precedence.
...
function TennisMatchSession({ params }: { params: TennisMatchParams }) {
  ...
  usePreventRemove(isFinalizing, ({ data }) => {
    ...
    setNotification({
      type: 'error',
      title: 'Saving Match',
      message: 'Please wait until the current match operation finishes.',
    });
  });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        if (!matchId)
          throw new Error(
            'Match ID is missing. Return to Matches to open a match.'
          );
        ...
        setDraftError(
          isAxiosError(error)
            ? 'Unable to verify the saved match. Please check your connection and retry.'
            : error instanceof Error
              ? error.message
              : 'Unable to restore the match. Please retry.'
        );
  ...
  useEffect(() => {
    if (
      ...
      readOnly ||
      acceptedRef.current ||
      isFinalizationInProgressRef.current
    )
      return;
    void matchDraftService
      ...
      .catch(() => {
        if (mountedRef.current)
          setDraftError(
            'Local save failed. Keep this match open and retry saving.'
          );
      });
  }, [
    matchId,
  ...
  const finalizeMatch = useCallback(async () => {
    ...
    if (!matchId || !state.matchWinner || events.length === 0) {
      setNotification({
        type: 'error',
        title: 'Cannot Save Match',
        message: 'A completed match and its recorded events are required.',
      });
      return;
    }
    ...
    try {
      await matchDraftService.saveDraft({
        matchId,
        state,
        events,
        elapsedSeconds,
      ...
      submitted = true;
      await matchFinalizationService.finalize(matchId, {
        final_state: {
          ...
          tiebreak_player1_points: state.tiebreakPlayer1Points,

          tiebreak_player2_points: state.tiebreakPlayer2Points,

          match_winner: state.matchWinner === 'PLAYER1' ? 'PLAYER1' : 'PLAYER2',

          completed_sets: state.completedSets.map((set) => ({
            player1_games: set.player1Games,

            player2_games: set.player2Games,

            was_tiebreak: set.wasTiebreak,

            tiebreak_player1_points: set.tiebreakPlayer1Points,

            tiebreak_player2_points: set.tiebreakPlayer2Points,

            serving_state: set.servingState
        ...
        events: chronologicalEvents.map((event, index) => ({
          ...
          recorded_at: new Date(event.recordedAt).toISOString(),
        })),
      });
      ...
      const uncertain =
        submitted && (status === undefined || status >= 500 || status === 409);
  ...
  }, [enterBrowserFullscreen, exitBrowserFullscreen, isBrowserFullscreen]);

  // RESTART MATCH
  const restartCurrentMatch = useCallback(() => {
    if (!canMutate()) return;
    resetMatch();
    resetTimer();
  ...
  }, [canMutate, resetMatch, resetTimer]);
  ...
  const resetCurrentMatch = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current)
        setDraftError(
          'Unable to reset the stored match. Your current session has been kept.'
        );
    } finally {
      isFinalizationInProgressRef.current = false;
      if (mountedRef.current) setIsFinalizing(false);
  ...
  useEffect(() => {
    if (state.matchWinner && timerStatus === 'running') {
      stopTimer();
    }
  }, [state.matchWinner, timerStatus, stopTimer]);
  ...
  const { player1Display, player2Display } = getDisplayPoints(
    state.player1Points,
    state.player2Points
  );
  ...
  const playerNameForId = (player: PlayerId) => {
    ...
    if (player === 'PLAYER2')
      return matchType === 'DOUBLES' ? player2Name : player2Name;
  ...
  return (
    ...
    >
      ...
      {draftStatus !== 'ready' && (
        <View style={{ padding: 12 }}>
          {draftStatus === 'loading' ? (
            ...
            <>
              <Text style={nativeTextDefaults}>{draftError}</Text>
              <TouchableRipple rippleColor="transparent" underlayColor="transparent" onPress={() => setLoadAttempt((value) => value + 1)}>
                <Text style={nativeTextDefaults}>Retry loading match</Text>
              </TouchableRipple>
            </>
          )}
        </View>
      ...
      <TennisHeader
        ...
        matchCompleted={Boolean(state.matchWinner)}
        ...
        onRestartMatch={restartCurrentMatch}
      ...
      >
        ...
        <MatchBoard
          ...
          onAddPoint={(player) =>
            scoringEnabled && addPoint(player, elapsedSeconds)
          }
          onMatchAction={(action, player) =>
            scoringEnabled && recordMatchAction(action, player, elapsedSeconds)
          }
          onUndo={() => {
          ...
          onFinalizeMatch={
            readOnly || acceptedRef.current || uncertainRef.current
              ? undefined
              : finalizeMatch
          }
