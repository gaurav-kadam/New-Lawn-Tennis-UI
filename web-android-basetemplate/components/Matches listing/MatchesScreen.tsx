...
export default function MatchesScreen() {
  ...
  const { tournament_code } = useLocalSearchParams<{
  ...
  const handleStartMatch = async (match: any) => {
    ...
    try {
      ...
      const team1Code = String(match.team1_code ?? match.team1 ?? '');
      const team2Code = String(match.team2_code ?? match.team2 ?? '');
      
      const [whiteResponse, blueResponse] = await Promise.all([
        team1Code
          ? teamService.getPlayersByTeamCode(team1Code)
          : Promise.resolve(null),

        team2Code
          ? teamService.getPlayersByTeamCode(team2Code)
          : Promise.resolve(null),
      ]);
      ...
      const team1Players = getPlayersFromResponse(whiteResponse);

      const team2Players = getPlayersFromResponse(blueResponse);
      ...
      const explicitPlayer3 =
        match.player3_name ?? match.player3Name ?? getPlayerName(match.player3);

      const explicitPlayer4 =
        match.player4_name ?? match.player4Name ?? getPlayerName(match.player4);
      ...
      if (matchType === 'DOUBLES') {
        player1Name =
          player1Name || getPlayerName(team1Players[0]) || 'Player 1';

        player2Name =
          player2Name || getPlayerName(team1Players[1]) || 'Player 2';

        player3Name =
          player3Name || getPlayerName(team2Players[0]) || 'Player 3';

        player4Name =
          player4Name || getPlayerName(team2Players[1]) || 'Player 4';
      } else {
        player1Name =
          player1Name || getPlayerName(team1Players[0]) || 'Player 1';

        player2Name =
          player2Name || getPlayerName(team2Players[0]) || 'Player 2';
      ...
      router.push({
        ...
        params: {
          ...
          player3Name: String(player3Name),
          player4Name: String(player4Name),
          matchType: matchType,
          matchFormat: String(
            match.match_format ?? match.matchFormat ?? 'BEST_OF_3'
          ).toUpperCase(),

          // MATCH INFORMATION
          matchNo: String(match.match_no ?? ''),

          courtNo: String(match.court_no ?? '1'),

          // DOUBLES SERVICE ORDER
          serviceOrder: JSON.stringify(
            match.service_order ?? match.doublesServeOrder ?? []
          ),

          servingState: JSON.stringify(match.serving_state ?? null),
        },
      });
    } catch (error) {
  ...
  const openEditModal = async (match: any) => {
    setEditingData({
      ...
      age_category: match.age_category ?? 'OPEN',
      gender: match.gender ?? 'Men',
      match_type: match.match_type ?? match.matchType ?? 'SINGLES',
      match_format: match.match_format ?? match.matchFormat ?? 'BEST_OF_3',
      team1: match.team1 ?? '',
      team2: match.team2 ?? '',
      ...
      is_complete: match.is_complete ?? false,
    });
  ...
  return (
    ...
    >
      <View
        style={{
          paddingHorizontal: isMobile ? tokens.spacing.md : tokens.spacing.xl,
          paddingTop: 15,
          flexShrink: 0,
        }}
      >
